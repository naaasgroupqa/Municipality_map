...
import { Play, Pause, RotateCcw, Search, Landmark, Map as MapIcon, Moon, LocateFixed, Route, Layers3, X, Images, Eye } from 'lucide-react';
import './styles.css';

const LOGO_URL = '/ministry-logo.png?v=20260906c';
...
function baseStyle(mode,opacity){return {version:8,sources:{base:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,attribution:'© OpenStreetMap contributors'}},layers:[{id:'base',type:'raster',source:'base',paint:{'raster-brightness-max':mode==='dark'?0.52:1,'raster-saturation':mode==='dark'?-0.82:0,'raster-contrast':mode==='dark'?0.18:0,'raster-opacity':opacity}}]};}
...
function fitQatar(map,duration=0){map.resize();map.fitBounds(QATAR_BOUNDS,{padding:{top:34,bottom:34,left:64,right:64},duration,maxZoom:7.9});}

function HeritageMap({sites,year,mode,showRoads,mapOpacity,onSelect,onReady,selectedId}){
 ...
 useEffect(()=>{
  const map=new maplibregl.Map({container:el.current,style:baseStyle(mode,mapOpacity),center:[51.18,25.35],zoom:6.5,minZoom:3.5,maxZoom:19,attributionControl:true});
  map.addControl(new maplibregl.NavigationControl({showCompass:false}),'bottom-right');mapRef.current=map;
  map.on('load',()=>{
   fitQatar(map,0);setTimeout(()=>fitQatar(map,0),220);
   ...
   map.addLayer({id:'roads-history-glow',type:'line',source:'roads',layout:{visibility:showRoads?'visible':'none'},paint:{'line-color':'#b31649','line-width':9,'line-opacity':0.16}});
   map.addLayer({id:'roads-history',type:'line',source:'roads',layout:{visibility:showRoads?'visible':'none'},paint:{'line-color':['case',['<',['get','year'],2000],'#d3a65d','#8a1538'],'line-width':3.5,'line-opacity':0.9}});
   map.addSource('gps',{type:'geojson',data:emptyPoint()});
   map.addLayer({id:'gps-halo',type:'circle',source:'gps',layout:{visibility:showRoads?'visible':'none'},paint:{'circle-radius':13,'circle-color':'#fff','circle-opacity':0.24,'circle-blur':0.3}});
 ...
 useEffect(()=>{const m=mapRef.current;if(!m?.getLayer('base'))return;m.setPaintProperty('base','raster-brightness-max',mode==='dark'?0.52:1);m.setPaintProperty('base','raster-saturation',mode==='dark'?-0.82:0);m.setPaintProperty('base','raster-contrast',mode==='dark'?0.18:0);},[mode]);
...
function PhotoPopup({site,onClose}){const [images,setImages]=useState([]),[loading,setLoading]=useState(true),[active,setActive]=useState(0);useEffect(()=>{let ok=true;setLoading(true);setActive(0);fetchCommonsPhotos(site).then(v=>{if(ok){setImages(v);setLoading(false);}});return()=>{ok=false};},[site.id]);return <div className="photo-popup"><button className="photo-close" onClick={onClose}><X size={18}/></button>{loading?<div className="photo-loading">Loading place photos…</div>:images.length?<><img className="photo-main" src={images[active]} alt={site.name}/><div className="photo-thumbs">{images.map((src,i)=><button key={src} className={i===active?'active':''} onClick={()=>setActive(i)}><img src={src} alt={`${site.name} ${i+1}`}/></button>)}</div></>:<div className="photo-empty"><Images size={28}/><b>No verified Commons photos found</b><span>Official Ministry/Qatar Museums images can be added later.</span></div>}<div className="photo-caption"><span>{site.category}</span><h3>{site.name}</h3><p>{site.municipality} · {site.period}</p></div></div>}

function App(){
 const [year,setYear]=useState(2026),[playing,setPlaying]=useState(false),[mode,setMode]=useState('road'),[showRoads,setShowRoads]=useState(true),[mapOpacity,setMapOpacity]=useState(0.72),[query,setQuery]=useState(''),[category,setCategory]=useState('All'),[selectedId,setSelectedId]=useState(null),[photoSite,setPhotoSite]=useState(null);const mapObj=useRef(null);
 ...
 return <div className="app-shell heritage-app">
  ...
  <main className="heritage-grid">
   ...
   <section className="map-panel"><HeritageMap sites={filtered} year={year} mode={mode} showRoads={showRoads} mapOpacity={mapOpacity} selectedId={selectedId} onSelect={chooseSite} onReady={m=>{mapObj.current=m;fitQatar(m,0)}}/><div className="map-overlay-title"><span>QATAR HERITAGE MAP</span><strong>{filtered.length}</strong><em>visible places</em></div><div className="map-switch"><button className={mode==='road'?'active':''} onClick={()=>setMode('road')}><MapIcon size={15}/> Map</button><button className={mode==='dark'?'active':''} onClick={()=>setMode('dark')}><Moon size={15}/> Presentation</button><button className={showRoads?'active':''} onClick={()=>setShowRoads(v=>!v)}><Route size={15}/> Road history</button><button onClick={resetQatar}><LocateFixed size={15}/> Full Qatar</button></div><div className="opacity-control"><div><Eye size={15}/><span>Map opacity</span><b>{Math.round(mapOpacity*100)}%</b></div><input aria-label="Map opacity" type="range" min="0.15" max="1" step="0.05" value={mapOpacity} onChange={e=>setMapOpacity(Number(e.target.value))}/></div><div className="map-help"><Layers3 size={13}/> Reduce map opacity to make heritage dots and historical roads clearer.</div>{photoSite&&<PhotoPopup site={photoSite} onClose={()=>setPhotoSite(null)}/>}</section>
   <aside className="detail-panel">{selected?<><span className="detail-kicker">{selected.category}</span><h2>{selected.name}</h2><h3>{selected.ar}</h3><div className="detail-meta"><span><b>Municipality</b>{selected.municipality}</span><span><b>Period</b>{selected.period}</span></div><p>{selected.summary}</p><button className="locate-btn" onClick={()=>chooseSite(selected.id)}><Images size={15}/> View photos</button></>:<div className="detail-placeholder"><Landmark size={30}/><h2>Select a historical place</h2><p>Click a marker on the map to view its details and photos.</p></div>}<div className="detail-note">Historical place data can later be replaced or expanded with official Ministry GIS and archive records.</div></aside>
  </main>
